    ...
    <body class="{{ app()->getLocale() }} login-template web wow WotLK" data-embedded-state="STATE_LOGIN">
        <div class="grid-container wrapper">
            ...
            <div class="input-container" id="login-wrapper">
                <form action="{{ route('login') }}" class=" username-required input-focus" id= "password-form" method="post" name="password-form">
                    {{ csrf_field() }}
                    ...
                    <div class="persistWrapper">
                        <label class="checkbox-label css-label">
                            <input type="checkbox" name="remember" {{ old('remember') ? 'checked' : '' }}>
                            <span class= "input-checkbox"></span>Remember Me
                        </label>
                    </div>

                    <div class="control-group submit">
                        <button class="btn btn-primary btn-large btn-block"
                        data-loading-text="" id="submit" tabindex="1" type= "submit">Login <i class="spinner-battlenet"></i></button>
                    </div>
                    <ul id="help-links">
                        <li>
                        ...
                        <li>
                            <a class="" href= "{{ route('password.request') }}" rel="external" tabindex="1">Forgot Your Password? <i class= "icon-external-link"></i></a>
                        </li>
                    </ul>
                </form>
            </div>
