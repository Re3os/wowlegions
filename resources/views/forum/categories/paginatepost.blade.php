...
<a class="Pagination-button Pagination-button--ordinal" href="{{ $url }}" data-page-number="{{ $page }}"><span class="Button-content">{{ $page }}</span></a>
                @endif
            @endforeach
            <!--div class="Dropdown Pagination-menu-container"> <span class="Pagination-ellipsis" data-trigger="toggle.dropdown.menu" type="button" data-toggle="tooltip" data-tooltip-content="Перейти к странице:"> <span class="Button-content">...</span> </span> <div class="Pagination-Menu Dropdown-menu"> <span class="Dropdown-arrow Dropdown-arrow--up" data-attachment="top right" data-target-attachment="bottom center"></span> <div class="Dropdown-items"><span class="Dropdown-item Dropdown-item--disabled label">Перейти к странице:</span> <form class="Form Dropdown-item Dropdown-item--disabled"> <div class="Pagination-input"> <input name="page" type="number" pattern="[0-9]" min="1" max="30" value="2" autocomplete='off' /> <div class="Input-border"></div> </div> </form> </div> </div> </div-->
        @endif
    @endforeach
