	...
	initialize: function() {
		...
		if (authLink.length > 0) {
			authLink.unbind().click(function() {
				Marketing.trackImpression('Battle.net Authenticator', 'Menu Toggle', $('#auth-menu').is(':hidden') ? 'Open' : 'Close');			
			...
			$('#auth-menu a').click(function() {
				...
				Marketing.trackImpression('Battle.net Authenticator', 'Link Click', label);
			...
			Marketing.trackImpression('Battle.net Authenticator', 'Impression', 'Impression');
		}
...
var Core = {
	 ...
	 */
	supportUrl: '../../../support/default.htm',
	 ...
	 */
	staticUrl: '../../../default.htm',
	sharedStaticUrl: '../../../local-common/default.htm',
...
var Feedback = {
	...
	initialize: function () {
		...
		this.feedbackUrl = '../../../' + Core.locale + '../../../feedback/default.htm';
			...
			if ($.trim(emailField.value) === '') {
				for (var i = 0, iLen = serializedForm.length; i < iLen; i += 1) {
					if (serializedForm[i].name === 'email') {
						serializedForm[i].value = 'no_email_given_40blizzard.com';
			}
				}
			}
...
var Login = {
	...
	*/
	_timeoutHandler: {
		...
		timeoutVar: null, /* stores the window.timeout reference so we can clear it later */
		loaderVar: null, /* store a reference to the spinner div */
		activate: function() {
			this.loaderVar = $("<div/>", {id: "embedded-loader"}).appendTo("body");
		...
		Search.ta = TypeAhead.factory(searchField || '#search-field', {
			...
			source: function(term, display) {
				$.ajax({
					...
					cache: true,
					dataType: url.charAt(0) === '../../../default.htm' ? 'json' : 'jsonp',
					success: function(response) {
						var results = [];
...
var TypeAhead = {
	...
	highlight: function(label, string) {
		...
		for (var i = 0, regex; regex = regexs[i]; i++) {
			...
			string = label.substr(0, l) + '<em>' + label.substr(l, r) + '</em>' + label.substr(l+r);
		}
		...
		for (var i = 0, type; type = config.resultTypes[i]; i++) {
			...
			if (groups[type]) {
				...
				for (var x = 0, item; item = groups[type][x]; x++) {
					...
					if (counter >= config.resultsLength) {
						break;
					} else if (config.termMatch && searchTerm.toLowerCase().indexOf(term.toLowerCase()) <= -1) {
						continue;
					}

					title = '<span class="title">' + this.highlight($('<div/>').text(item.title).html(), searchTerm) + '</span>';
